...
export function createDefaultCounterContexts(playerCount: number): CounterContext[] {
    return calculateCounterOffsets(playerCount).map((angle) => ({
...
export function calculateCounterOffsets(playerCount: number): number[] {
    if (playerCount % 4 == 0) {
        ...
        return new Array(playerCount).fill(0).map((_, i) => halfStep + (i * 360) / playerCount);
    }
    return new Array(playerCount).fill(0).map((_, i) => (i * 360) / playerCount);
}
