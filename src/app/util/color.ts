...
export function darken(color: Color, angle = 30) {
    ...
    {
        return { h: color.h + COLOR_SHIFT * shades, s: color.s, l: color.l - DARKER * shades };
    }
...
export function conicGradient(degrees: number, baseColor: HSL, backgroundColor: Color): string {
    ...
    if (angle > 0 || Object.is(+0, angle)/* we differentiate ±0 */) {
        // forwards
        return `conic-gradient(${backgroundColor} 0deg, ${HSL2String(darken(baseColor, angle))} 0deg, ${HSL2String(baseColor)} ${angle}deg, ${backgroundColor} ${angle}deg)`;
    } else {
        // backwards
        const backAngle = angle + 360;
        return `conic-gradient(${backgroundColor} ${backAngle}deg, ${HSL2String(baseColor)} ${backAngle}deg, ${HSL2String(darken(baseColor, angle))} 360deg, ${backgroundColor} 360deg)`;
    }
}